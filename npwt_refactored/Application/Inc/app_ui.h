...
#include <stdint.h>
#include <stdbool.h>
#include "../../Core/Inc/system_config.h"  // 系统配置（包含ENABLE_CONTINUOUS_ADJUST等）
...
/**
 * @brief UI系统状态（对应老版本工程的mod_main_a）
 */
typedef enum {
...
typedef struct {
    ...
    bool continuous_adjust_active;  // 是否正在连续调节
    uint8_t continuous_adjust_key_id; // 当前调节的按键ID（1=上键，2=下键）
#endif
