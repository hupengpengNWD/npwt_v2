...
#include  "npwt_con_main.h"
 ...
 unsigned short adc_ps0,adc_bat = BAT4;// lwz adc_ps0传感器的压力值
 unsigned short adc_zero;// lwz 当前大气压的值，该值在开机时就进行采集，这个数值被用来计算参考值，如果采集的气压稍有变化，可能会导致当前气压的变化。也会影响气压上限的计算，例如，如果采集到的初始气压大于143，则有可能会导致最大气压达不到320
/*
...
void ADC_DatCal(void)/*used*/
{
	...
	if (++delay_P_adc > 3)//每隔3ms采集一次：总共采集3次，计9ms取平均值
	{
		adc_ps0 = adc_reader_press();
		delay_P_adc = 0;
	}
	// adc_bat = bat_scan_filter(bat_temp);
	if (++delay_adc > 250)//每隔250ms采集一次,总共采集10次，计2.5S取平均值
	{
