...
# 71 "../../source/adc.c"
static short value_press_buf[3];
...
static unsigned short adc_press_filter(unsigned char ch)
{
 ...
 if ( PS_i == 3 )PS_i = 0;
 for ( count=0;count<3;count++)sum += value_press_buf[count];
 sum=(unsigned short)(sum/3);
  if(sum>1023)sum=1023;
  if(sum<1)sum=0;
   return (unsigned short)sum;
}

static short value_power_buf[10]={
315,315,315,315,315,
315,315,315,315,315,
};
...
static unsigned short adc_power_filter(unsigned char ch)
{
 ...
 if ( Pw_i == 10 )Pw_i = 0;
 for ( count=0;count<10;count++)sum += value_power_buf[count];
